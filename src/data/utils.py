""" Function to get sentence pairs.
"""
...
def get_sentence_pairs(original_sentence_pairs, dataset):
    ...
    new_training_set["text_source"] = original_sentence_pairs.iloc[current_source_id, :]["text_source"].reset_index(
        drop=True)
    new_training_set["text_target"] = original_sentence_pairs.iloc[current_target_id, :]["text_target"].reset_index(
        drop=True)
    new_training_set["source_id"] = current_source_id
    new_training_set["target_id"] = current_target_id
    new_training_set['Translation'] = new_training_set.apply(lambda row: int(row['source_id'] == row['target_id']),
                                                             axis=1)
